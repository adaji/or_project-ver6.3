...
from . import flight_model4_param_toler as flight_model
import numpy as np
...
class DownhaulView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        problem_names = [
            ...
            'airline_daily_max',
            'airline_total_share',
            'airline_total_share_tolerance',
            'total_daily_slot_limit',
            'daily_pass_limit_min',
            'daily_pass_limit_max',
            'daily_slot_cap_small_cities',
            'daily_slot_cap_mid_cities',
            'first_days_by_first_airline'
        ]

        try:
            prob_args = [json.loads(data[x]) for x in problem_names]
            # Constraints Data
            med_first_min_perc = json.loads(data['med_first_min_perc'])
            med_first_max_perc = json.loads(data['med_first_max_perc'])
            unavailable_flights_in_routes = json.loads(data['unavailable_flights_in_routes'])
            unavailable_flights_in_routes = {
                int(k): v for k, v in unavailable_flights_in_routes.items()}
            flights_daily_cap = json.loads(data['flights_daily_cap'])
            flights_daily_cap = {int(k): v for k, v in flights_daily_cap.items()}
            glued_flights = json.loads(data['glued_flights'])
            small_stations_routes = json.loads(data['small_stations_routes'])
            mid_stations_routes = json.loads(data['mid_stations_routes'])
            first_priority_routes = json.loads(data['first_priority_routes'])
            last_priority_routes = json.loads(data['last_priority_routes'])
        ...
        constr = flight_model.Constraints(
            med_first_min_perc=med_first_min_perc,
            med_first_max_perc=med_first_max_perc,
            unavailable_flights_in_routes=unavailable_flights_in_routes,
            flights_daily_cap=flights_daily_cap,
            glued_flights=glued_flights,
            small_stations_routes=small_stations_routes,
            mid_stations_routes=mid_stations_routes,
            first_priority_routes=first_priority_routes,
            last_priority_routes=last_priority_routes
        )

        try:
            # Making new and clean model - preventing over writing when several models run one after the other
            flight_model.opt_model = flight_model.gp.Model(name="Downhaul Flight MIP Model")
            ...
            if flight_model.solcnt > 0:
                ...
                flight_model.opt_model.write("./Output/Parvaz/downhaul_solution.sol")
                flight_model.visualize(prob, sol_x, sol_xres,
                                       './Output/Parvaz/downhaul_solution.xlsx')
...
def buildResponse(optimal=0, inf=0, final=False):
    if flight_model.solcnt == 0:
        ...
        solution = {
            ...
            'daily_flight_med': json.dumps(daily_flight_med),
            'daily_flight_toler_med': json.dumps(daily_flight_toler_med),
            'actual_daily_flight_med': json.dumps(actual_daily_flight_med),

            'daily_flight_mec': json.dumps(daily_flight_mec),
            'daily_flight_toler_mec': json.dumps(daily_flight_toler_mec),
            'actual_daily_flight_mec': json.dumps(actual_daily_flight_mec),

            'total_pass_med': json.dumps(sum(actual_daily_flight_med)),
            'total_pass_mec': json.dumps(sum(actual_daily_flight_mec)),
            'med_last_downhaul_start_day': json.dumps(flight_model.med_last_downhaul_start_day)
        }
...
def makeSolList(final=False):
    ...
    if final:
        ...
        reserve[:, int(dim3/2)] = prob.jed_staff_reserve
...
class DownhaulExcelView(APIView):
    def get(self, request, *args, **kwargs):
        response = HttpResponse(
            content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = "attachment; filename=Downhaul_Solution.xlsx"
