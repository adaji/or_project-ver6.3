...
class Takhsis:
    def __init__(self,
                 ...
                 items_weight=[[192, 191, 190, 189, 188, 187, 186, 185, 184, 183, 182, 181, 180, 179, 178, 177, 176, 175, 174, 173, 172, 171, 170, 169, 168],
                               ...
                               [1]],
                 items_value=[0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.54, 0.36, 0.36, 0.36,
                              0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.36, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, -2],
        ...
        self.items_grouping = items_grouping
        self.items_weight = items_weight
        self.items_value = items_value
        self.target_items_perc = target_items_perc
...
def create_takhsis_model(prob=Takhsis()):
    ...
    items_weight_raw = [prob.items_weight[i][j]
                        for i in range(len(prob.items_weight)) for j in range(len(prob.items_weight[i]))]
    items_weight_index = [[items_weight_raw.index(prob.items_weight[i][j]) for j in range(
        len(prob.items_weight[i]))] for i in range(len(prob.items_weight))]

    num_weights = len(items_weight_raw)

    # DEBUG
    # print("prob.med_first_group")
    # print(prob.med_first_group)
    # print("prob.mec_first_group")
    # print(prob.mec_first_group)
    print("downhaul_grouping_mfirst")
    print(downhaul_grouping_mfirst)

    first_packs = []  # size will be 28
    for d in range(first_days):
        # downhaul_grouping_mfirst_index refers to what stations has flight on day d
        # downhaul_grouping_mfirst refers to the daily passengers for day d on each station
    ...
    print("prob.med_last_group")
    print(prob.med_last_group)
    print("prob.mec_last_group")
    print(prob.mec_last_group)
    print("backhaul_grouping_mlast")
    print(backhaul_grouping_mlast.tolist())
    ...
    opt_model.addConstrs(gp.quicksum(x[d, w, p, i] * items_weight_raw[w]
    ...
    if prob.items_grouping == 0:
        ...
        tot_pass = sum(map(sum, prob.med_first_group)) + sum(map(sum, list(prob.med_last_group)))
        num_carevan_groups = len(prob.items_weight)

        items_error = {(g): opt_model.addVar(vtype=GRB.INTEGER, lb=-20000000, name='ERR(%i)' % (g))
                       for g in range(num_carevan_groups)}

        opt_model.addConstrs(items_error[g] == gp.quicksum(x[d, w, 1, i] * items_weight_raw[w]
                                                           for d in range(first_days+last_days)
                                                           for w in items_weight_index[g]
                                                           for i in range(num_items_per_pack[d][1])
                                                           ) - round(prob.target_items_perc[g] * tot_pass)
                             for g in range(num_carevan_groups))

        items_abs_error = {(g): opt_model.addVar(vtype=GRB.INTEGER, name='ABS_ERR(%i)' % (g))
                           for g in range(num_carevan_groups)}
        opt_model.addConstrs(items_abs_error[g] == gp.abs_(items_error[g])
                             for g in range(num_carevan_groups))

        # Optimization weight-large weight for carevans with size 1 - 1 2 3 for 190-135-90 - 90 error is more important
        ww = list(set(range(1, num_carevan_groups))) + [100]
        tot_abs_error = gp.quicksum(items_abs_error[g]*ww[g] for g in range(num_carevan_groups))
        # END NEW OBJECTIVE
        penalty_for_1 = gp.quicksum(x[d, num_weights-1, 1, i]
                                    for d in range(first_days+last_days) for i in range(num_items_per_pack[d][1]))

        opt_model.ModelSense = GRB.MINIMIZE
        opt_model.setObjective(tot_abs_error+penalty_for_1)

    # opt_model.write("out-param.lp")
    # opt_model.write("out-param-fin.mps")

    # Model Parameters
    opt_model.Params.timelimit = 15000
    ...
    # opt_model.write("./Output/Takhsis/Alloc.sol")
...
def visualize(
    ...
    # Make a raw items weight with no grouping
    items_weight = [grouped_items_weight[i][j]
                    for i in range(len(grouped_items_weight)) for j in range(len(grouped_items_weight[i]))]
    ...
    for h in range(hotel_count):
        for d in range(prob1.tot_days):
            for i in range(max(d-prob1.stay_duration+1, 0), min(d+1, len(prob1.daily_flight_cap))):
    ...
    for h in range(hotel_count):
        for d in range(prob2.tot_days):
            for i in range(max(d-prob2.stay_duration+1, 0), min(d+1, len(prob2.daily_flight_cap))):
    ...
    for d in range(len(prob1.daily_flight_cap)):
        for h in range(hotel_count):
            total_entry1[d] += entry_list1[h, d]
    # Med Last
    total_entry2 = np.zeros(prob2.tot_days, dtype=int)
    for d in range(len(prob2.daily_flight_cap)):
        for h in range(hotel_count):
    ...
    for i in range(prob1.tot_days):
        if i <= len(prob1.daily_flight_cap)-1:
    ...
    for i in range(prob2.tot_days):
        if i <= len(prob2.daily_flight_cap)-1:
    ...
    for h in range(hotel_count):
        ...
        for d in range(prob1.tot_days):
            if d >= len(prob1.daily_flight_cap) or entry_list1[h, d] == 0:
                ws.write(row+h*4+4, 3+d, "", empty_hotel_fmt)

            if stay_list1[h, d] < 1:
                continue
            if d < len(prob1.daily_flight_cap) and entry_list1[h, d] >= 1:
        ...
        # Above for med last
        for d in range(prob2.tot_days):
            if d >= len(prob2.daily_flight_cap) or entry_list2[h, d] == 0:
                ws.write(row+h*4+4, 3+med_first_backhaul_start_day +
                         d, "", empty_hotel_fmt)

            if stay_list2[h, d] < 1:
                continue
            if d < len(prob2.daily_flight_cap) and entry_list2[h, d] >= 1:
    ...
    # Day number
    for i in range(len(prob1.daily_flight_cap)):
    ...
    main_mec_groups, final_mec_groups = build_mec_groups(
        total_entry1[:len(prob1.daily_flight_cap)].tolist(), jeddah_flights_med_first)

    print()
    print(total_entry1)
    print("prob1.daily_flight_cap")
    print(prob1.daily_flight_cap)
    ...
    row += 1
    ws.write(row, 0, 'طول سفر', mec_group_title_fmt)
    ws.write(row, 1, 'روز خروج', mec_group_title_fmt)
    ws.write(row, 2, 'گروه', mec_group_title_fmt)
    row += 1

    # Running Build Mecca Groups Algorithm for Med Last

    # total_entry2[:-prob2.stay_duration+1].tolist())
    main_mec2_groups, final_mec2_groups = build_mec_groups(
        jeddah_flights_med_last, prob2.daily_flight_cap)
    global final_med2_groups
    global med_last_tot_duration
    global med_last_group_num

    print("Attention: main_mec2_groups")
    print(list(jeddah_flights_med_last))
    print(prob2.daily_flight_cap)
    print(main_mec2_groups)
    # Grouping just for index
    main_mec2_groups_index, _ = build_mec_groups(
        prob2.daily_flight_cap, jeddah_flights_med_last)
    temp_row = copy.deepcopy(row)
    for i in range(len(main_mec2_groups_index)):
        for j in range(len(main_mec2_groups_index[i])):
            med_last_group_num.append([i+1, j+1])
            ws.write(temp_row, 1, i+1, mec_group_fmt)
            ws.write(temp_row, 2, j+1, mec_group_fmt)
            temp_row += 1
    # End grouping just for index
    ...
    for i in range(len(main_mec2_groups)):
        for j in range(len(main_mec2_groups[i])):
            ...
            ws.write(row, 0, med_last_tot_duration[-1], duration_fmt)
            # REMOVED GROUPING INDEX-MOVED ABOVE
            # iran green 1
    ...
    print("ATTENTION:final_med2_groups len must be 12")
    print(final_med2_groups)
    ...
    row += 5
    for d in range(len(prob2.daily_flight_cap)):
        dd = d+len(prob1.daily_flight_cap)
...
def flight_sol_grouping(x, xres, med_staff_reserve, jed_staff_reserve, flight_capacity, downhaul_mfirst):
    X = np.array(x)
    XRes = np.array(xres)
    ...
    if downhaul_mfirst:
        reserve[:, 0] = med_staff_reserve
        reserve[:, int(dim3/2)] = jed_staff_reserve
    else:
        reserve[:, 0] = jed_staff_reserve
        reserve[:, int(dim3/2)] = med_staff_reserve

    for d in range(dim2):
        for r in range(dim3):
            dr = 0
            for f in range(dim1):
                dr += X[f, d, r] * (flight_capacity[f] - reserve[d, r]) - XRes[f, d, r]

            daily_pass_route[d, r] = dr
    print("daily_pass_route")
    print(daily_pass_route)
    if downhaul_mfirst:
        print("down")
        print(daily_pass_route[:, :int(dim3/2)])
        return daily_pass_route[:, :int(dim3/2)]
    else:
        res = daily_pass_route[:, int(dim3/2):]
        print("back")
        print(res[np.sum(res, axis=1).nonzero()])
